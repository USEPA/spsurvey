...
#'   the survey design used. Only returns the relevant non-null output.
#'
...
grtspts_returnused <- function(sframe, stratum = NULL, seltype = "equal", nsamp, caty.n = NULL, 
  ...
  if(length(seltype) == length(stratum)) {
    ...
    # sapply will make this a vector - I think this is easiest for users to digest
  ...
  # create output list
  # returns elements whose sub-elements have at least one non-null value
  for (s in names(dsgn)) {
    if (length(Filter(Negate(is.null), dsgn[[s]])) == 0) dsgn[[s]] <- NULL
  }

  sites <- list(sites.base = sites.base, sites.over = sites.over, sites.near = sites.near,
                dsgn = dsgn)
  # removes parent level null elements
  sites <- Filter(Negate(is.null), sites)

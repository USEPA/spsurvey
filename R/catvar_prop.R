...
catvar_prop <- function(z, wgt, x, y, prop, prop_names, stratum.ind,
  stratum.level, cluster.ind, clusterID, wgt1, x1, y1, pcfactor.ind, fpcsize,
...
# Calculate the weighted residuals matrix
      ...
      n <- length(z.lst[[i]])
      im <- tapply(wgt2.lst[[i]], z.lst[[i]]) == matrix(rep(1:m, n), nrow = n,
        byrow = TRUE)
...
# Calculate the variance estimate
    ...
    if(vartype == "Local") {
      ...
      varest <- (pcfactor*apply(total2est * matrix(rep(wgt1.u, m),
        ...
        apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)) / tw2
    } else {
      varest <- NULL
    }
...
# Calculate the variance estimate
    ...
    if(vartype == "Local") {
      ...
      varest <- pcfactor*apply(rm, 2, localmean_var, weight.lst) / tw2
    } else {
      varest <- NULL
    }

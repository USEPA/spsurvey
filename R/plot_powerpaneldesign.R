...
#'   function power_dsgn.
#'
#' @param plot_type   Default is "standard" which plots standard power curve. If
#'   equal to "relative", then plot power of one panel design compared to one or
#'   more other panel designs.
#'
#' @param trend_type Character value for trend in mean ("mean") or or percent
#'   change in proportion ("percent") of cumulative distribution function above
#'   or below a fixed cut point.  Default is "mean".
#'
#' @param xaxis_type Character value equal to "period" or "change" which
#'   designates the type of x-axis for power plot where power is plotted on
#'   y-axis.  For xaxis_type = "period", x-axis is periods in dsgnpower. If
#'   xaxis_type = "change", then x-axis is percent per period with secondary
#'   x-axises for total percent per period and associated change in mean.
#'   Default is "period".  Note that xaxis_type controls how the input for
#'   "period" and "trend" paramenters is used.
#'
#' @param comp_type  Character value equal to "design" or "Indicator" which
#'   designates the type of power curve comparison that will occur on a single
#'   plot.  If comp_type = "design", then on a single plot of power curves all
#'   panel designs specified in "dsgns" are plotted for a single indicator,
#'   single trend value and single alpha.  If comp_type = "indicator", then on a
...
#'   dsgnpower. If xaxis_type is equal to "period", then NULL results in maximum
#'   trend value being used and a single value or vector of values results in a
#'   separate plot for each value specified.  If xaxis_type is equal to
...
#'   dsgnpower. If xaxis_type is equal to "period", then NULL results in all
...
#'   value specified being plotted on x-axis. If xaxis_type is equal to
...
#' Power_tst <- power_dsgn("Variable_Name", ind_values = 43, unit_var = 280,
...
#' \dontrun{
#' pdf("Power_tst.pdf")
#' plot_powerpaneldesign(Power_tst, plot_type = "relative", comp_type = "design",
...
plot_powerpaneldesign <- function(dsgnpower, plot_type = "standard",
  ...
  if (!is.null (period)) {
    ...
    if (xaxis_type == "period") {
      ...
      if (length (period) == 1) { period_values <- dsgnpower$period}
      period_min <- min (period_values)
      period_max <- max (period_values)
        ...
        for (k in 1:n_ind) {
          ...
          if (xaxis_type == "period") {
            for (i in 1:n_trend) {
              ...
              plot (xset, seq(0,1,length=length(xset)), ylim=c(0,1), xlim=c(period_min, period_max),
              ...
              for (m in dsgn_plot[1:n_dsgn]) {
                pow <- dsgnpower$dsgn_power[m , as.character(period_values),
          ...
          if (xaxis_type == "change") {
            for (i in 1:n_period) {
              ...
              xset_mean <- dsgnpower$trend.change[paste ("Trend_", round (x, 1), "%", sep = "") ,
              ...
              ltype <- 1
              for (m in dsgn_plot[1:n_dsgn]) {
                pow <- dsgnpower$dsgn_power[m , as.character(period_plot[i]),
        ...
        for (k in 1:n_dsgn) {
          ...
          if (xaxis_type == "period") {
            for (i in 1:n_trend) {
              ...
              for (m in ind_plot[1:n_ind]) {
                pow <- dsgnpower$dsgn_power[dsgn_plot[k] , as.character(period_values),
          ...
          if (xaxis_type == "change") {
            for (i in 1:n_period) {
              ...
              xset_tot <- (period_plot[i] -  period_min + 1) * xset
              if (n_ind == 1) {
                xset_mean <- dsgnpower$trend.change[paste ("Trend_", x, "%", sep = "") ,
                                            as.character(period_plot[i]), ind_plot[k]]
                xlabel <- paste ("Trend: %/period; Total % and Mean at Period ", period_plot[i], sep="")
              }
              else {
              ...
              ltype <- 1
              for (m in ind_plot[1:n_ind]) {
                pow <- dsgnpower$dsgn_power[dsgn_plot[k] , as.character(period_plot[i]),
        ...
        for (k in 1:n_ind) {
          ...
          if (xaxis_type == "period") {
            for (i in 1:n_trend) {
              ...
              base_pow <- dsgnpower$dsgn_power[dsgn_plot[1], as.character(period_values),
                                               ind_plot[k], trend_plot[i], alpha_plot[j] ]
              for (m in dsgn_plot) {
                pow <- dsgnpower$dsgn_power[m, as.character(period_values), ind_plot[k],
              ...
              xset <- seq (period_min, by=1, to=period_max)
              # set up initial plot area,  xaxis and yaxis
              plot (xset, seq(0,1,length=length(xset)), ylim=c(ylow, 1), xlim=c(period_min, period_max),
                    ylab="", xlab="", type="n", axes=F)
              axis (side = 1, line = -0.75, at = xset, labels = xset, adj = 0.5, font = 3, cex = 1)
              ...
              text (period_min, 0.86, font=3, cex=.8, adj=0, paste0('Indicator = ', ind_plot[k] ) )
              # legend for panel design power plotted
              legend (period_min, 0.82, c(dsgn_plot[1], dsgn_plot), lty=c(1,1:n_dsgn),
          ...
          if (xaxis_type == "change") {
            for (i in 1:n_period) {
              ...
              ymin <- - 0.2
              # find panel_base power and plot power curve
              base_pow <- dsgnpower$dsgn_power[dsgn_plot[1], as.character(period_plot[i]),
              ...
              # legend for panel design power plotted
              legend (xmin, 0.82, c(dsgn_plot[1], dsgn_plot), lty=c(1,1:n_dsgn),
                      col = c("blue", rep("black", length(dsgn_plot))), lwd=2, seg.len=5, bty="n")
        ...
        for (k in 1:n_dsgn) {
          ...
          if (xaxis_type == "period") {
            for (i in 1:n_trend) {
              ...
              ymin <- -0.2
              # find panel_base power and plot power curve
              base_pow <- dsgnpower$dsgn_power[dsgn_plot[k], as.character(period_values),
              ...
              # legend for panel design power plotted
              legend (period_min, 0.82, c(ind_plot[1], ind_plot), lty=c(1,1:n_ind),
          ...
          if (xaxis_type == "change") {
            for (i in 1:n_period) {
              ...
              ymin <- -0.2
              # find panel_base power and plot power curve
              base_pow <- dsgnpower$dsgn_power[dsgn_plot[k], as.character(period_plot[i]),
              ...
              # legend for indicator power plotted
              legend (xmin, 0.82, c(ind_plot[1], ind_plot), lty=c(1,1:n_ind),
                      col = c("blue", rep("black", length(ind_plot))), lwd=2, seg.len=5, bty="n")

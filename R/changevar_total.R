...
changevar_total <- function(catvar.levels, catvar1, catvar2, wgt, x, y,
  revisitwgt, size1, size2, stratum.ind, stratum.level, cluster.ind, clusterID,
        ...
        if(var.ind[i]) {
          if(vartype == "Local") {
            weight.lst <- localmean_weight(x2.lst[[i]], y2.lst[[i]],
              1/wgt2.lst[[i]])
            var2est[i,] <- as.vector(pcfactor*localmean_cov(im, weight.lst))
          } else {
            var2est[i,] <- as.vector(pcfactor*n*var(im))
            if(SRSind)
              vartype <- "Local"
          }
        }
...
# Calculate covariance or correlation estimates
      ...
      if(vartype == "Local") {
        ...
        varest <- pcfactor*localmean_cov(im, weight.lst)
      } else {

###############################################################################
# Function: changevar_total (exported)
...
changevar_total <- function(catvar_levels, catvar1, catvar2, wgt, x, y,
  revisitwgt, size1, size2, stratum_ind, stratum_level, cluster_ind, clusterID,
...
# Calculate the weighted indicator matrix
        ...
        im <- cbind(z1, z2) * matrix(rep(wgt2_1st[[i]], 2), nrow=n)
        ...
        if(var_ind[i]) {
          if(vartype == "Local") {
            weight_1st <- localmean_weight(x2_1st[[i]], y2_1st[[i]],
              1/wgt2_1st[[i]])
            var2est[i,] <- as.vector(pcfactor*localmean_cov(im, weight_1st))
          } else {
...
# Calculate covariance or correlation estimates
      ...
      if(vartype == "Local") {
        ...
        varest <- pcfactor*localmean_cov(im, weight_1st)
      } else {

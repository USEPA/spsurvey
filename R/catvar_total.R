...
catvar_total <- function(z, wgt, x, y, size_names, stratum.ind, stratum.level,
...
# Calculate the weighted indicator matrix
      ...
      n <- length(z.lst[[i]])
      im <- cbind(tapply(wgt2.lst[[i]], z.lst[[i]]) == matrix(rep(1:(m-1), n),
        nrow = n, byrow = TRUE), rep(1,n)) * matrix(rep(wgt2.lst[[i]], m),
        nrow = n)
      ...
      if(var.ind[i]) {
        if(vartype == "Local") {
          weight.lst <- localmean_weight(x2.lst[[i]], y2.lst[[i]],
            1/wgt2.lst[[i]])
          var2est[i,] <- pcfactor*apply(im, 2, localmean_var, weight.lst)
        } else {
...
# Calculate the variance estimate
    ...
    if(vartype == "Local") {
      ...
      varest <- pcfactor*apply(im, 2, localmean_var, weight.lst)
    } else {
      varest <- NULL
    }

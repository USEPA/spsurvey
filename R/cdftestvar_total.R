...
cdftestvar_total <- function(design, wgt, x, y, stratum.ind,
...
# Create the model matrix for the contingency table using using a single cluster
      ...
      design.var <- subset(design$variables, cluster == cluster.levels[i] &
          !is.na(rowvar))
      mm_cells <- model.matrix(frm_cells, model.frame(frm_cells, design.var,
        na.action = na.pass))
      if(length(rowlev) > 1) {
        mm_rows <- model.matrix(frm_rows, model.frame(frm_rows, design.var,
          na.action = na.pass))
      } else {
        mm_rows <- rep(1, NROW(mm_cells))
      }
      mm_cols <- model.matrix(frm_cols, model.frame(frm_cols, design.var,
        na.action = na.pass))
      mm_total <- rep(1, NROW(mm_cols))
...
# Calculate the weighted residuals matrix
      ...
      rm <- mm_cluster * matrix(rep(wgt2.lst[[i]], m_cl), nrow=n)
...
# Calculate the variance estimate
    ...
    if(vartype == "Local") {
      ...
      varest <- (pcfactor*localmean_cov(total2est * matrix(rep(wgt1.u, m_cl),
        ...
        matrix(rep(wgt1.u, m_cl^2), nrow=ncluster), 2, sum), nrow=m_cl))
    } else {
...
# Calculate the variance estimate
    ...
    if(vartype == "Local") {
      ...
      varest <- pcfactor * localmean_cov(rm, weight.lst)
    } else {

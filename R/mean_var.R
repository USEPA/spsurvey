...
mean_var <- function(z, wgt, x, y, mean_est, stratum_ind, stratum_level,
  ...
  if (cluster_ind) {
    ...
    for (i in 1:ncluster) {
      ...
      n <- length(z_1st[[i]])
      rv_mean <- wgt2_1st[[i]] * (z_1st[[i]] - mean_est)
      ...
      if (var_ind[i]) {
        if (vartype == "Local") {
          ...
          var2est[i] <- pcfactor * localmean_var(rv_mean, weight_1st)
        } else {
    ...
    if (vartype == "Local") {
      ...
      varest <- pcfactor * localmean_var(total2est * wgt1_u, weight_1st) /
        (tw^2)
    } else {
      varest <- NULL
    }
    ...
    if (vartype == "Local") {
      weight_1st <- localmean_weight(x, y, prb = 1 / wgt)
      varest <- pcfactor * localmean_var(rv_mean, weight_1st) / (tw^2)
    } else {
      varest <- NULL
    }

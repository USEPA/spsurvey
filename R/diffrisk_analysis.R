...
# Date: July 12, 2021
#
#' Risk Difference Analysis for Probability Survey Data
#'
#' This function organizes input and output for risk difference analysis of
#' categorical data generated by a probability survey.  Risk difference measures
#' the strength of association between stressor and response variables by
#' calculating the difference between poor condition of the response variable
#' given poor condition of the stressor variable and poor condition of the
#' response variable given good condition of the stressor variable, where
#' stressor and response variables are classified as either good (i.e.,
#' reference condition) or poor (i.e., different from reference condition).
#' The input data argument, dframe, can be either a data frame or a simple
#' features (sf) object.  If an sf object is used, coordinates are extracted
#' from the geometry column in the object, arguments xcoord and ycoord are
#' assigned values "xcoord" and "ycoord", respectively, and the geometry column
...
#'   names of response variables in the \code{dframe} data frame. Each response
...
#'   names of stressor variables in the \code{dframe} data frame. Each stressor
#'   variable must have two category values (levels), where one level is
#'   associated with poor condition and the other level is associated with good
#'   condition.
#'

    ...
    if(is.null(weight1)) {
      ...
      if(!is.null(stratumID)) {
        ...
        for(i in 1:nstrata) {
          ...
          if(any(sapply(tapply(temp.weight1[[i]], temp_clusterID, unique),
            length) > 1)) {
            error_ind <- TRUE
      ...
      if(is.null(sweight1)) {
        ...
        if(!is.null(stratumID)) {
          ...
          for(i in 1:nstrata) {
            tst <- stratumID == stratum.levels[i]
            temp_clusterID <- clusterID[tst]
            if(any(sapply(tapply(temp.sweight1[[i]], temp_clusterID, unique),
              length) > 1)) {
              error_ind <- TRUE
      ...
      if(is.null(xcoord1) || is.null(ycoord1)) {
        ...
        if(!is.null(stratumID)) {
          ...
          for(i in 1:nstrata) {
            tst <- stratumID == stratum.levels[i]
            temp_clusterID <- clusterID[tst]
            if(any(sapply(tapply(temp_xcoord1[[i]], temp_clusterID, unique),
              length) > 1)) {
              error_ind <- TRUE
              msg <- paste0("\nThe stage one x-coordinate must be constant for all stage two sampling units within \neach stage one sampling unit of stratum ", stratum.levels[i], ".\n")
              error_vec <- c(error_vec, msg)
            }
            if(any(sapply(tapply(temp_ycoord1[[i]], temp_clusterID, unique),
              length) > 1)) {
              error_ind <- TRUE

...
test_that("one sided formulas work", {
  ...
  expect_error(plot(NE_Lakes, formula = ~ ELEV_CAT, varlevel_args = varlevel_args), NA)
  expect_error(plot(NE_Lakes, formula = ~ ELEV_CAT,
                    varlevel_args = varlevel_args, var_args = var_args, pch = 19), NA)
})
...
test_that("plotting works", {
  ...
  expect_error(plot(eqprob_rho, formula =  ~ siteuse,
                    varlevel_args = varlevel_args, var_args = var_args, cex = 0.5), NA)
  var_args <- list(ELEV_CAT = list(siteuse = list(levels = c("Base", "Over"), pch = c(1, 19))))
  expect_error(plot(eqprob_rho, formula = siteuse ~ ELEV_CAT,
                    var_args = var_args, onlyshow = "low", cex = 0.5), NA)
})
...
test_that("plotting works", {
  var_args <- list(siteuse = list(main = "maintest"))
  varlevel_args <- list(siteuse = list(levels = c("Base", "Over"), pch = c(1, 19)))
  expect_error(plot(eqprob_rho, NE_Lakes, formula =  ~ siteuse,
                    varlevel_args = varlevel_args, var_args = var_args, cex = 0.5), NA)
  var_args <- list(ELEV_CAT = list(siteuse = list(levels = c("Base"), pch = c(19))))
  expect_error(plot(eqprob_rho, NE_Lakes, formula = siteuse ~ ELEV_CAT,
                    var_args = var_args, onlyshow = "low", siteuse = c("Base"), cex = 1), NA)
})

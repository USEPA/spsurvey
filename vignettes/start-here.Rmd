...
Next we discuss three ways to construct sf objects in R. The first is to read a shapefile directly into R using `sf::st_read()` or `sf::read_sf()`. The second is to use the `sf::st_sf()` function or the `sf::st_as_sf()` function to combine an appropriate R object (most commonly a data frame) and an appropriate geometry object into an `sf` object. To illustrate one approach for turning a data frame into an `sf` object, we start `NE_Lakes_df`, a data frame in spsurvey that contains variables and geographic coordinates (latitude and longitude coordinates) for lakes in the Northeastern United States. To turn `NE_Lakes_df` into `NE_Lakes_geo`, an `sf` with geographic coordinates, run
```{r}
...
You can move between coordinate systems using `sf::st_transform()`. For example, we can transform `NE_Lakes_geo` (which uses geographic coordinates) to `NE_Lakes` (which uses projected coordinates) by running
```{r}

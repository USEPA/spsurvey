...
These vignettes cover some of the core functions (and arguments within those functions) in spsurvey. To learn more about features of spsurvey that are not covered in these vignettes, we encourage you to read spsurvey's documentation available for download [here](https://cran.r-project.org/web/packages/spsurvey/index.html). Help files for a particular function are viewable by running `?function_name` after loading spsurvey. For example, to learn more about the `grts()` function, run `?grts`.

...
1. Summarizing and Visualizing Data: Use the `sp_summary()` and `sp_plot)` functions to summarize and visualize sample frames, samples, and design frames. To view this vignette, run `vignette("EDA", "spsurvey")` (EDA is short for exploratory data analysis).
...
Next we discuss three ways to construct sf objects in R. The first is to read a shapefile directly into R using `sf::st_read()` or `sf::read_sf()`. The second is to use the `sf::st_sf()` function or the `sf::st_as_sf()` function to combine an appropriate R object (most commonly a data frame) and an appropriate geometry object into an `sf` object. We can turn `NE_Lakes_df`, a data frame in spsurvey that contains variables and coordinates for lakes in the Northeastern United States, into an `sf` object named `NE_Lakes` by running
```{r}
NE_Lakes <- st_as_sf(NE_Lakes_df, coords = c("XCOORD", "YCOORD"), crs = 5070)
NE_Lakes
```

The `coords` argument to `st_as_sf` specifies the columns in `NE_Lakes_df` that are the x-coordinates and y-coordinates. The `crs` argument specifies the coordinate reference system, which we discuss in more detail next.
...
You can move between coordinate systems using `sf::st_transform()`. For example, we can transform from projected to geographic coordinates (latitude and longitude) in `NE_Lakes`by running
```{r}
NE_Lakes_geo <- st_transform(NE_Lakes, crs = 4326)
NE_Lakes_geo
```

Then we can transform back to projected coordinates by running
```{r}

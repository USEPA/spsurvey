...
* `ip` is the sites original inclusion probability
* `wgt` is the sites survey design weight
...
* `caty.n` is a vector with sample sizes for each group in `caty_var`
...
* `seltype` is `"equal"` for an equal probability design
* `n.samp` is 60, the desired sample size
* `legacy_option` is set to `TRUE` to accomodate legacy sites

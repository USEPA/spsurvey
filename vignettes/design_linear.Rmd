...
Throughout this vignette, you will use select probability samples from the `Illinois_River` dataset. The `Illinois_River` dataset is made up of 9087 line segments from the Illinois River in Arkansas and Oklahoma. To load the data into R, run
...
```{r}
boundbox <- st_bbox(Illinois_River)
xmin <- boundbox["xmin"]
xmax <- boundbox["xmax"]
ymin <- boundbox["ymin"]
ymax <- boundbox["ymax"]
...
plot(st_geometry(Illinois_River), xlim = c(xmin, xmax), ylim = c(ymin, ymax), cex = 0.5)
```
...
eqprob <- grts(sframe = Illinois_River, seltype = "equal", n.samp = 60)
```
...
Suppose we wish to sample river segments in Oklahoma with a higher probability than river segments in Arkansas. To select an unequal probability GRTS sample by state, run
```{r}
# expected sample size for each group with names
caty.n <- c(Arkansas = 20, Oklahoma = 40)
# selecting the unequal probability sample
uneqprob <- grts(sframe = Illinois_River, seltype = "unequal", 
                          n.samp = 60, caty_var = "STATE_NAME", 
                          caty.n = caty.n)
```
...
```{r}
# names of the strata
strata <- c("Arkansas", "Oklahoma")
# sample sizes of the strata
strata.n <- c("Arkansas" = 30, "Oklahoma" = 30) # suggested change -- 
#remove strata argument and just use the names of strata.n
...
low_pop <- subset(Illinois_River, STATE_NAME == "Oklahoma")
# subset the sample to include ma lakes
low_samp <- subset(strat_eqprob$sites.base, STATE_NAME == "Oklahoma")
...
```{r}
# equal probability design
eqprob_irs <- irs(sframe = Illinois_River, seltype = "equal", n.samp = 60)
```
...
```{r}
eqprob_legacy <- grts(Illinois_River, seltype = "equal", n.samp = 60,
...
mindis <- 1600
eqprob_mindis <- grts(Illinois_River, seltype = "equal", n.samp = 60,
                         mindis = mindis)
```
...
```{r}
eqprob_rho <- grts(Illinois_River, seltype = "equal", n.samp = 60,
                         n.over = 10)
```
...
```{r}
eqprob_nn <- grts(Illinois_River, seltype = "equal", n.samp = 60,
                         n.near = 10)
```
...
```{r, fig.show = "hold", fig.align = "center", fig.cap = "Sampled sites (aquamarine) and replacement sites (orange) for an equal probability GRTS design using nearby neighbors for the replacement sites."}
# gray background
par(bg = "gray95")
# plotting the population sites
plot(st_geometry(Illinois_River), main = "", reset = FALSE, 
...
```{r}
# compute for grts
pielou_grts <- spbalance(sample = eqprob$sites.base, population = Illinois_River)
pielou_grts
# compute for irs
pielou_irs <- spbalance(sample = eqprob_irs$sites.base, population = Illinois_River)
pielou_irs
```

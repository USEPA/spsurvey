---
title: "Selecting Spatially Balanced Samples from a Linear Network (Infinite Population)"
...
output: 
  html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Selecting Spatially Balanced Samples from a Linear Network (Infinite Population)}
...
The next few lines describe variables in `Illinois_River`; for more regarding the `Illinois_River` data and associated variables, run `?Illinois_River`.

We'll later summarize and visualize the `Illinois_River` data. But before doing so, we need store the data as an `sframe` (short for "sample frame") object:
```{r}
Illinois_River <- sframe(Illinois_River)
```

Next we give an overview of GRTS designs and visualize the `Illinois_River` data.
...
* `sframe` (sample frame) is the `Illinois_River` data.
...
Suppose we wish to sample river segments in Oklahoma with a higher probability than river segments in Arkansas. First it is helpful to summarize river segments in each state.
```{r}
...
* `caty_var` matches the name of the variable in `Illinois_River` with the unequal probability groups.
...
* `stratum_var` matches the name of the variable in `Illinois_River` containing the strata.
...
```{r, fig.align = "center", fig.cap = "Population (sframe) and sampled sites (sites_base) obtained for the two `STATE_NAME` strata using a stratified GRTS design with equal selection probabilities within strata."}
...
The standard GRTS algorithm may select sites that are closer together than desired. You can adjust the algorithm to require a minimum distance between sites.  Supppose we requires sites be 1,600 meters (1 mile) apart. To include this requirement in an unstratified, equal probability design, run
```{r}
...
```{r, fig.align = "center", fig.cap = "Population (`sframe`) and sampled (`sites_base`) sites for an equal probability GRTS design having a minimum distance between sites of 1,600 meters."}
...
```{r}
eqprob_rho <- grts(Illinois_River, seltype = "equal", n_base = 60, n_over = 10)
```

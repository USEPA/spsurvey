...
The sampling frame contains all units that may be selected in the sample. Summarizing and visualizing the sampling frame is often helpful to better understand your data and inform additional sampling options (e.g. stratification). To use `sp_plot()` or `sp_summarize()`, sampling frames must either be an `sf` object or a data frame with x-coordinate, y-coordinate, and crs (coordinate reference system) information.
...
If a two-sided formula is used, it is possible to adjust graphical parameters left-hand side variables for all levels of a right-hand side variable. This occurs when a sublist matching the structure of `varlevel_args` is used as an argument to `var_args`. In this next example, different shapes are used for the small and large levels of `AREA_CAT` for all levels of `ELEV_CAT`:
```{r}
...
Similar to `sp_summary()` and `sp_plot()` for sampling frames, `sp_summary()` and `sp_plot()` for samples uses a formula. For samples, the default formula is `~siteuse`, which summarizes or visualizes each `sites` object from the design sites (`eqprob_rho`). By default, the formula is applied to all non-`NULL` `sites` objects from the design sites. In this example, those are `sites_base` (for the base sites) and `sites_over` (for the reverse hierarchically ordered replacement sites).
```{r}
...
The `sp_summary()` and `sp_plot()` functions are quite useful when you include `siteuse` as a left-hand side variable (`siteuse` is treated as a categorical variable), as you can summarize and visualize each `sites` object for each level of each right-hand side variable:
```{r}
...
`sp_summarize()` or `sp_plot()` work for analysis data the same way they do for sampling frames.
The `NLA_PNW` data in spsurvey is analysis data (as an `sf` object) from lakes in California, Oregon, and Washington. There are two variables in `NLA_PNW` you use next:

1. `STATE`: The state name (California, Washington, Oregon)
2. `NITR_COND` : Nitrogen content categories (poor, fair, good)

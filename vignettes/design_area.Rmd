---
title: "Selecting Spatially Balanced Samples from an Areal Network (Infinite Population)"
...
output: 
  html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Selecting Spatially Balanced Samples from an Areal Network (Infinite Population)}
...
The next few lines describe variables in `Great_Lakes`; for more regarding the `Great_Lakes` data and associated variables, run `?Great_Lakes`.

We'll later summarize and visualize the `Great_Lakes` data. But before doing so, we need store the data as an `sframe` (short for "sample frame") object:
```{r}
Great_Lakes <- sframe(Great_Lakes)
```

Next we give an overview of GRTS designs and visualize the `Great_Lakes` data.
...
* `sframe` (sample frame) is the `Great_Lakes` data.
...
There are 4113 total polygons spread out over the five water bodies: 858 in Lake Erie, 1636 in Lake Huron, 525 in Lake Michigan, 187 in Lake Ontario, and 907 in Lake Superior.

Next, you can plot the shorelines for each water body:
...
The `key.length` argument simply streches the legend to fit all the text for the water body categories.

To select an unequal probability GRTS sample by water body, run
```{r}
...
* `seltype` is "unequal" for an unequal selection probabilities.
...
* `stratum_var` matches the name of the variable in `Great_Lakes` containing the strata.
...
```{r, fig.align = "center", fig.cap = "Population (sframe) and sampled sites (sites_base) obtained for the two `WTBDY_NM` strata using a stratified GRTS design with equal selection probabilities within strata."}
plot(
  strat_eqprob,
  Great_Lakes,
...
We can alter the previous design to accomodate proportional probability sampling within strata where each strata's sample is proportional to polygon (in square kilometers; `AREA_SQKM`); larger shorelines have a higher probability of being sampled. These types of designs are useful when the response variable you plan to collect is positively correlated with the auxiliary variable; in this case, the proportional probability design is **more efficient** than a design with equal selection probabilities within strata. Before implementing this design, we can summarize total `AREA_SQKM` within each `WTBDY_NM` category:
```{r}
...
* `stratum_var` matches the name of the variable in `Great_Lakes` containing the strata.
* `n_base` is a names vector whose names identify strata and whose values inidicate the within-strata sample sizes.
* `seltype` is "proportional" for proportional selection probabilities within strata.
* `aux_var` matches the name of the variable in `Great_Lakes` to sample proportionally by.
    * This variable must be *strictly positive* (all shorelines have positive areas)
...
The standard GRTS algorithm may select sites that are closer together than desired. You can adjust the algorithm to require a minimum distance between sites.  Supppose we requires sites be 500 meters apart. To include this requirement in an unstratified, equal probability design, run
```{r}
...
* `seltype` is `"equal"` for equal selection probabilities.
* `n_base` is 60, the desired sample size.
* `mindis` is the minimum distance desired between sites.
...
```{r, fig.align = "center", fig.cap = "Population (`sframe`) and sampled (`sites_base`) sites for an equal probability GRTS design having a minimum distance between sites of 500 meters."}
...
* `seltype` is `"equal"` for equal selection probabilities.
* `n_base` is 60, the desired sample size.
* `n_over` is the number of nearest neighbor replacement sites desired for each base site.

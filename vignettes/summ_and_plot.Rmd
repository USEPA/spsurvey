...
```
For numeric variables, '`summary()` provides a few simple numeric summaries. To visualize `ELEV`, run
```{r}
...
The `NLA_PNW` data in spsurvey is a design frame as an `sf` object that contains lakes from California, Oregon and Washington sampled in an NLA study. There are eight variables in `NLA_PNW`:

1. `SITE_ID`: lake site identification
2. `WEIGHT`: weight from survey design
3. `URBAN`: lake categorized as urban or non-urban
4. `STATE`: state in which lake is located
5. `BMMI` : Benthic macroinvertebrate multi-metric index
6. `BMMI_COND` : BMMI categorized into condition classes
7. `PHOS_COND` : Total phosphorus categorized into condition classes
8. `NITR_COND` : Total nitrogen categorized into condition classes

```{r}
NLA_PNW <- dframe(NLA_PNW)
summary(NLA_PNW, formula = ~ NITR_COND + BMMI)
plot(NLA_PNW, formula = ~ NITR_COND + BMMI)
```
Visualizing design frames that are not sf objects requires specification of coordinates in the `plot()` arguments.

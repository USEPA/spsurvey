...
`var_args` uses `list1` to give the `AREA_CAT` visualization a new title and color palette; `varlevel_args` uses `list2` to give the `ELEV_CAT` visualization different shapes for the small and large levels; and `...` uses `cex = 0.75` to reduce the size of all points and `key.width` to adjust legend spacing. 

...
The `onlyshow` argument is a useful argument to spsurvey's `summary()` and `plot()` functions.

Other arguments to the `summary()` and `plot()` functions in spsurvey can be found in the help files, which are viewed by running `?summary()` and `?plot()` after loading spsurvey.

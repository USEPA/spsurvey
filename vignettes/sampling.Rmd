...
The `NE_Lakes` sample frame in spsurvey is used in this vignette to introduce spatially balanced sampling. The `NE_Lakes` sample frame is finite and contains lakes (treated as a whole) from the Northeastern United States. There are four variables in `NE_Lakes` used in this vignette:
...
So far this vignette has broadly mentioned spatial balance but has not yet discussed how to empirically measure it. Stevens and Olsen (2004) proposed measuring spatial balance using Voroni polygons. A Voroni polygon for a sampled site contains all sites in the sample frame closer to this sampled site than any other sampled site. The voroni polygons are combind with a loss function to measure spatial balance. Several spatial balance metrics are available in spsurvey, but the default is Pielou's evenness index. Pielou's evenness index ranges from 0 (perfect spatial balance) to 1 (a complete lack of spatial balance). Larger values of the Pielou's evenness index indicate worse spatial balance. To measure the spatial balance of the unstratified GRTS and IRS samples with equal inclusion probabilities, run
```{r}

...
We focus mainly on using the `NE_Lakes` sample frame in spsurvey to introduce spatially balanced sampling. The `NE_Lakes` sample frame is finite and contains lakes (treated as a whole) from the Northeastern United States. There are four variables in `NE_Lakes` that we will use:
...
`pch = 19` uses closed circles instead of open circles, and `key.width = lcm(3)` adjusts the legend margin so all of the text fits nicely within the plot's borders.
...
`seltype` is `"proportional"` to indicate proportional inclusion probabilities, and `aux_var` is the proportional probability variable (`"AREA"` in `NE_Lakes`). Proportional (to size) inclusion probabilities are useful because they can increase the precision of estimates when the response variable is positively correlated with the positive auxiliary variable.
...
Sometimes sites are selected to be sampled but this sampling cannot occur. Sampling may not occur for a variety of reasons, some of which include landowner denial or a lack of funding. The `grts()` function has options for selecting replacement sites, which are sites available to replace sites originally selected but that cannot be sampled. The first replacement site option is the reverse hierarchical ordering option (Stevens and Olsen, 2004). To select a base sample with replacement sites selected by reverse hierarchical ordering, run
```{r}
...
To select an IRS sample stratified by elevation category with equal inclusion probabilities witihin strata, run
```{r}
strata_n <- c(low = 25, high = 25)
...
So far this vignette has broadly mentioned spatial balance but has not yet discussed how to empirically measure spatial balance. Stevens and Olsen (2004) proposed measuring spatial balance using Voroni polygons. A Voroni polygon for a sampled site contains all sites in the sample frame closer to this sampled site than any other sampled site. Loss functions are then constructed using these polygons to measure spatial balance. Several spatial balance metrics are available in spsurvey, but the default is Pielou's evenness index, which ranges from 0 (perfect spatial balance) to 1 (a complete lack of spatial balance). Larger values of the Pielou's evenness index indicate worse spatial balance. To measure the spatial balance of the unstratified GRTS and IRS samples with equal inclusion probabilities, run
```{r}
...
To select a GRTS sample with equal inclusion probabilities from `Great_Lakes`, run
```{r}
eqprob <- grts(Great_Lakes, n_base = 50)
```

To visualize the sample overlain onto the sample frame, run
```{r}
plot(eqprob, Great_Lakes, pch = 19, key.width = lcm(3))
```

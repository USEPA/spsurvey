...
Categorical variables are analyzed in spsurvey using the `cat_analysis()` function. The `cat_analysis()` function estimates the proportion of observations and the total extent (area) that belongs to each level of the categorical variable. Several useful pieces of output are returned by `cat_analysis()`, including estimates, standard errors, margins of error, and confidence intervals. The `.P` suffix correspond to estimates of proportions for each category, while the `.U` estimates correspond to estimates of totals for each category. 

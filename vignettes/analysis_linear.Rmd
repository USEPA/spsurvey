---
title: "Analyzing Probability Samples for Linear Resources (Infinite Population)"
output: 
  rmarkdown::html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Analyzing Probability Samples for Linear Resources (Infinite Population)}
...
In this vignette, you will use spsurvey to analyze probability samples for the `NRSA_EPA7` data, which represents a subset of rivers from EPA's National Rivers and Streams Assessment (NRSA). There are two main analysis types this vignette covers: categorical variable analysis using the `cat_analysis()` function and continuous variable analysis using the `cont_analysis()` function. 
...
Then, you can load the `NRSA_EPA7` data by running 
```{r}
data("NRSA_EPA7")
```
...
```{r}
NRSA_EPA7 <- dframe(NRSA_EPA7)
```

Suppose we wanted to summarize the number of sampled lakes that were from the target population (`TNT_CAT`):
```{r}
summary(NRSA_EPA7, ~ TNT_CAT)
```

For more information regarding the `NRSA_EPA7` data, run `?NRSA_EPA7`.
...
* `dframe` is `NRSA_EPA7`, the design frame.
...
```{r}
NRSA_EPA7_Target <- na.omit(subset(NRSA_EPA7, TNT_CAT == "Target"))
```
...
* `dframe` is `NRSA_EPA7_Target`, the design frame for the target population.
...
To analyze the phosphorus condition class separately for the Eastern Temperate Forest and Great Plains ecoregions (`NA_LEVEL1_ECO_NM`), run
```{r}
phos_cond_subpop <- cat_analysis(
  dframe = NRSA_EPA7_Target,
  vars = "PHOSPHORUS_COND",
  subpops = "NA_LEVEL1_ECO_NM",
  weight = "WGT_TP_CORE",
  xcoord = "XCOORD",
  ycoord = "YCOORD",
  popcorrect = TRUE
...
* `subpops` is `"NA_LEVEL1_ECO_NM", the variable representing the subpopulation (ecoregions).
...
To analyze Benthic MMI (`BENTHIC_MMI`) for the Eastern Temperate Forest and Great Plains ecoregions (`NA_LEVEL1_ECO_NM`), run
```{r}

...
There are two commonly used ways to construct `sf` objects in R. The first is to read a shapefile directly into R using `st_read()` or `read_sf()`. The second is to use the `st_sf()` function or the `st_as_sf()` function to combine an appropriate R object (most commonly a data frame) and an appropriate geometry object into an `sf` object. For more information regarding `sf` objects, see Pebesma (2018) and Lovelace et al. (2019). A third way exists in `spsurvey` if your data is an `sp` object: Using the `sframe()` function.

...
This vignette covers plotting in `spsurvey` using the `NE_Lakes` data. Plotting in `spsurvey` largely builds on plotting functionality in `sf`, and any arguments you can normally specify in `sf` plotting system, you can specify in `spsurvey`'s. For more background, we recommend reading the `sf`'s [Plotting Simple Features](https://r-spatial.github.io/sf/articles/sf5.html) vignette and running `?plot.sf`.  This vignette is separated into two sections. The first, [Plotting Sample Frames], covers plotting your sample frame and is most useful before selecting your probability sample. The second, [Plotting Design Object] covers plotting your design object (sample frame and sampled sites) and should be used after a probability sample is selected.
...
Next we will visualize the `NE_Lakes` data. But before doing so, we need to store the data as an `sframe` (short for "sample frame") object:
```{r}
...
When plotting categorical variables, the `object` and `formula` arguments must be specified: `object` is your `sframe` object and `formula` provides the variables to be plotted. For now, we use a one-sided formula (`~ variables`) to visualize categorical variables. These types of plots can be useful to determine the spatial layout of strata or unequal probability groups before selecting a sample.
...
You will notice that this changes the symbol for **all points**. But what if you want to plot small, open circles for the low elevation level and large, `x`'s for the high elevation level? To do this, you must use the `varlevel_args` argument (short for a variable's **level** arguments) in `plot()`. `varlevel_args` is a named list containing sublists whose names equal each variable to be adjusted and whose values indicate the levels of the categorical variable as well as the associated graphical parameters.
```{r}
varlevel_args <- list(
  ELEVATION_CAT = list(
    levels = c("low", "high"),
...
If you specify a graphical parameter in this way, values must be provided **for all levels** of the variable. Later you will use the `var_args` argument to pass a single argument to all levels of a variable.
...
Certainly additional arguments like `var_args` or `varlevel_args` can be specified. Geometries can only be plotted when the left hand side of the formula is empty.
...
To visualize the sampled sites overlain on the sample frame, run
```{r}
...
To adjust graphical parameters, use `varlevel_args` -- here we plot the sample frame sites with open circles and the base sites with closed circles:
```{r}
varlevel_args <- list(
  siteuse = list(levels = c("sframe", "sites_base"), pch = c(1, 19))
)
...
If you want to plot each level of the `sites` argument by a categorical variable, similar to plots from [Categorical Response by Categorical Variables], run
```{r}
plot(sample, NE_Lakes,  ~ ELEV_CAT, pch = 19, key.pos = 1)
```
...
var_args <- list(
  ELEV_CAT = list(
    siteuse = list(levels = c("sframe", "Base"), pch = c(1, 19))
  )
)
plot(
  sample,
...
Now suppose you use `spsurvey` to select another sample of 200 while also specifying 100 replacement sites (using the reverse heirarchical ordering method) in case some of your original sites can't be sampled:
```{r}

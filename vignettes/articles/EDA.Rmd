...
These functions are exploratory tools, not estimation tools. They summarize and visualize the data without using design weights, so they describe the sampling frame or the sample itself rather than the entire population. Population estimates and their standard errors come from the analysis functions (see the `*_analysis()` functions discussed in the Analyzing Data vignette), which combine the data with the design weights.
...
Before summarizing or visualizing a sampling frame, turn it into an `sp_frame` object using `sp_frame()`:
```{r}
...
The counts returned here are counts of sampled sites, not estimates of how many lakes in the target population fall in each nitrogen condition category. Because the sites carry unequal design weights, the two generally differ. Use `cat_analysis()` to estimate the population proportions and totals, along with their standard errors and confidence intervals.
